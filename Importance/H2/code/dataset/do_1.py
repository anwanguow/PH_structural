...
time_list = np.array([100,200,300,400,500,600,700,800,900])

for set_ in set_n:
    for i in num:    
        for t in time_list:
            file_D = "PI/add/set_" + str(int(set_)) + "/D_" + str(int(i)) + "/features_" + str(int(t)) + ".npy";
            D = np.load(file_D, allow_pickle=True);
            new_matrix = normalize_columns(D);
            df = pd.DataFrame(new_matrix);
            output_name = "matlab/database/add/set_" + str(int(set_)) + "/D_" + str(int(i)) + "/X_t_" + str(t) + ".csv"
