...
set_dirs = ["PH/barcode/set_1", "PH/barcode/set_2"]
output_dirs = ["PI/decay/set_1", "PI/decay/set_2"]
...
for set_index, set_dir in enumerate(set_dirs):
    for traj_index in range(10):
        ...
        for frame in frames:
            ...
            target_path = f"data/p_hop_t/set_{set_index + 1}/D_{traj_index}/Y_{frame}.npy"
            target = np.load(target_path)
            feature_matrix = np.array(feature_matrix)
            target = target.reshape(-1, 1)
            feature_matrix = np.hstack((feature_matrix, target))
